<template>
  <section class="deliverySection" :class="{ arrivalOnly: hasReflections }" aria-label="다시 발견하기">
    <article v-if="!hasReflections" class="deliveryCard paperPanel">
      <div>
        <span class="screenEyebrow">재발견 루프</span>
        <h2>쓰는 순간보다 다시 만나는 순간을 설계합니다.</h2>
      </div>
    ...
    <article class="arrivalCard paperPanel">
      <span class="screenEyebrow">오늘 도착</span>
      <template v-if="arrivalReflectionId">
        <h2>{{ arrivalWindowLabel ?? "예전" }} 남긴 회고가 도착했어요</h2>
        <p>{{ arrivalPreview }}</p>
