...
// 받는 사람에게는 공유 라우트만 열린다. 지금 보고 있는 경로(/issues/<내 책장 id>)에 토큰을
// 붙이면 그 id는 상대 책장에 없어 가드가 표지로 되돌리고, 쿼리까지 사라진다.
function shareLink(id: string): string { return `${window.location.origin}/shared/${id}`; }
