...
- **혼자 쓰기 / 복간** — 솔로 발행, 종이 회고 옮겨 적기, 원본 보관 맥락 기록
...
- [DESIGN.md](./DESIGN.md) — 현재 에디토리얼 잡지형 디자인 시스템
...
- 읽기 상태는 4자리 코드로 조회 가능하다. 쓰기 액션(시작·질문·답변·추측·공개·마감)은 합류 시 발급되는 참가자 토큰이 필요하다.
- 개인 책장은 localStorage. 로컬 저장소 데이터는 프론트에서 shape 검증 후 불러온다.
- 답변은 **전원 제출 전 서버가 은닉**(`answers: null`), 추측·답변은 **1회 불변**(재제출로 점수 조작 불가).
...
현재 기준은 `DESIGN.md`, `docs/recoverse-spec.md`, `ARCHITECTURE.ko.md`, 그리고 실제 워크스페이스 코드다.

`DESIGN_SYSTEM.ko.md`, `PRODUCT_PLAN.md`, `PRODUCT_SPEC.ko.md`, `USER_FLOW.md`, `REDESIGN_PLAN.ko.md`, `UX_REDESIGN_BRIEF.ko.md`, `ACCOUNT_STORAGE_PLAN.ko.md`, `SECURITY.md`, `TODO.md`는 피벗 이전 방향이 섞여 있어 참고용으로만 본다.
