...
session:{code}:token:{name}          → 참가자 쓰기 액션 토큰
...
읽기 상태 조회는 4자리 코드로 가능하다. 쓰기 액션(시작·질문·답변·추측·공개·다음·마감)은 합류 시 발급된 참가자 토큰을 `name`과 함께 제출해야 한다. 친구·가족 범위의 가벼운 세션이라는 전제는 유지하지만, 코드와 이름만으로 다른 사람이나 호스트 행세를 하는 문제는 막는다.
...
- **토큰 탈취**: 참가자 토큰은 탭의 sessionStorage에 저장된다. 같은 브라우저 프로필이나 devtools 접근을 신뢰할 수 없는 환경까지 방어하지는 않는다.
